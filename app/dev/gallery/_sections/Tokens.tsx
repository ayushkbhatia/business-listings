// The token layer. Every swatch is a Tailwind utility mapped to a CSS variable in
// globals.css. If a value here is wrong, the token is wrong — there is nowhere
...
import { Section as KitSection } from "../_kit";
...
function Section({ id, children }: { id: string; children: React.ReactNode }) {
  return (
    <KitSection id={`token-${id.replace(/\s+/g, "-")}`} title={id}>
      {children}
    </KitSection>
  );
}
