...
| `/dashboard/analytics` | Wired | Board 3l, amended by the 3a/3l position work. Four panels over a daily rollup, every figure a comparison against the previous window. Fed by `/api/jobs/daily`; query ranks carry their denominator and their reason on the row rather than in a note under the table. **Stage 1 counts the results pages only.** The area and emirate landing pages list businesses and record no impressions, while a storefront view from anywhere counts in stage 2 — so "clicked through to your listing" understates its own denominator. Fixing it needs a source discriminator on `search_impression_day`: `normalised` is both the row's identity and the phrase shown under *"what buyers searched to reach you"*, so a landing impression written today would appear there as a phrase nobody typed and would feed the demand-gap matcher. It goes with the next migration. |
...
| `/dashboard/domain` | **Works** | Rewritten on 9 Sep 2026 from a bring-your-own-domain flow to a label under our own zone. No token and no per-seller provisioning: `proxy.ts` matches the host and rewrites to the storefront. Two things are still outside the repo and are what a browser needs to arrive — a wildcard `*.businesslistings.me` DNS record at the registrar, and that wildcard added to the Vercel project for the certificate. |
