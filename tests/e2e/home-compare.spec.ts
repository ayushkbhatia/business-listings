...
/**
 * Handoff 1, step 5. Home.
 *
 * The comparison tray this file also covered compared suppliers, by slug, in
 * the URL. Board `10d` retired it for a product comparison in a cookie; its
 * spec is compare-products.spec.ts.
 */
...
test.describe("axe", () => {
  /* `/compare` has its own pass in compare-products.spec.ts, where the ids are read off the seed. */
  for (const route of ["/"]) {
