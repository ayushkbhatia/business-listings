...
-- ─────────────────────────────────────────────────────────────────────────────
-- 5 · The message the waiting list owes
-- ─────────────────────────────────────────────────────────────────────────────
--
-- Board `11e` `B10`: *"the waitlist needs a position, a notification and a
-- release rule, or it is a dead button."* It had the position and, since D2, a
-- `notified_at` stamp the screen reads — so a seller who did not open the screen
-- learned nothing. The release rule ratified on 17 Sep is **everybody waiting
-- is told and the first to answer takes it**, which only works if everybody is
-- actually told.
--
-- `ADD VALUE IF NOT EXISTS` so a re-run is free. The value is added here and
-- used by nothing in this transaction, which is what Postgres requires.

ALTER TYPE "notification_event" ADD VALUE IF NOT EXISTS 'placement_slot_freed';

-- ─────────────────────────────────────────────────────────────────────────────
-- 6 · The two templates that message renders
-- ─────────────────────────────────────────────────────────────────────────────
--
...
-- `tests/unit/notification-templates.test.ts` reads both files and holds them in
-- step with `prisma/seed-notification-templates.mts`.
--
...
--
-- **After the `ALTER TYPE` above** — and in a separate statement from it,
-- because Postgres refuses to use a new enum value in the transaction that
-- added it. Prisma runs each migration file in one transaction, so these two
